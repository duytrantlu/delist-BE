  ...
  return passport.authenticate('local-signup', (err) => {
    if (err) {
      ...
      if (err.name === 'MongoError' && err.code === 11000) {
        // 11000 Mongo code is for a duplication username error
...
function validateSignupForm(payload) {
  ...
  if (!payload || typeof payload.username !== 'string' ||
      !(validations.username.regex.value).test(payload.username.trim())) {
    isFormValid = false;
    errors.username = validations.username.regex.message;
  }
